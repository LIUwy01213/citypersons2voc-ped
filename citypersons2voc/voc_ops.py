...
object_tmpl = \
    ...
    '''  # .format(lbl, truncated, x1, y1, x2, y2)
tail = '</annotation>'
...
@filter_val: {ft_val}
    ...
    def __write_xml(self, img_name, w, h, bbs):
        ...
        for bb in bbs:
            [lbl, x1, y1, x2, y2,truncated,zero] = bb
            content += object_tmpl.format(self.lbl_map[lbl],truncated, x1, y1, x2, y2)
        content += tail
    ...
    def __run(self, src_img_dir, name_bbs_dict, name_set, enable_filter):
        ...
        for k, vec in name_bbs_dict.items():
          ...
          #  print( len(bbs[:, 3] ))
            kn = len(bbs[:, 5])
            for i in range(0, kn):
                bbs[i, 5] = float(bbs[i, 5] * bbs[i, 6]) / float(bbs[i, 3] * bbs[i, 4])  # adding -1
                print(float(bbs[i, 5]))
